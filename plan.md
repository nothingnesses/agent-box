...
- **On creation (`ab new`):** Compute the slug. If the computed slug dir already exists and its sentinel points to the same repo, proceed (idempotent). If it points to a different repo, error with a message naming both conflicting repo paths:
  ...
  ```
  Then: create dir → write sentinel. **No scan of all slug dirs.** The "workspace exists under a different slug" case (from migration or hash algorithm change) is handled by `ab dbg migrate` and by the auto-heal scan fallback on lookup (`ab spawn`). Avoiding the O(n) scan keeps `ab new` fast regardless of how many workspace directories exist.
...
4. Otherwise (slug dir absent, or sentinel missing/mismatched) → scan all dirs under `{workspace_dir}/{type}/`, read each `.source-repo`. If found, **auto-heal**: atomically rename the old slug dir to the current hash (handle `ENOENT` for concurrent heal), return `Ok(Some(new_path))`.
...
**`new_workspace()`** (line 85-128): Change `repo_name: Option<&str>` to `repo_path: Option<&Path>`. Pass through to `resolve_repo_id()`. Compute workspace path using the hash-computed slug (no scan for existing workspaces under different slugs — that case is handled by `ab dbg migrate` and by the auto-heal scan fallback in `resolve_workspace_dir()` on lookup). Update `source_path()` calls to drop config where applicable. Still needs config for `workspace_dir`.
...
Output lists orphans and old-layout dirs with their slug names and (where available) original source paths. Accepts an optional positional `slug: String` argument — when provided, shows detailed info for that slug only: `.source-repo` contents, session directories, and whether the source repo still exists on disk. Flags:
- `--clean`: Remove all orphaned and old-layout directories found by the scan (or the specific slug if provided). Show the full list before prompting for interactive confirmation unless `--force` is also passed. Support `--dry-run`.
- `--force`: Skip interactive confirmation (requires `--clean`).
- `--dry-run`: Preview what `--clean` would delete without acting (requires `--clean`).

This consolidates orphan inspection and cleanup into a single command: `ab dbg orphans` for read-only inspection, `ab dbg orphans --clean` for cleanup, `ab dbg orphans --clean <slug>` for removing a specific slug. This is fewer commands to learn, and cleanup is discoverable via `--help`.

**`DbgCommands::Remove`** (line 123-132): Change `repo: String` to `repo_path: Option<PathBuf>`. When provided, pass to `RepoIdentifier::from_path()`, call `resolve_workspace_dir()` (the free function in `repo.rs`) to find the actual workspace directory (handles mismatched slugs), remove. When not provided, detect repo from CWD. Require interactive confirmation unless `--force` is passed. Keep `--dry-run` flag as-is. `Remove` stays focused on removing a specific repo's workspaces; orphan cleanup is handled by `Orphans --clean`.
...
**Old-layout and orphan detection:** No warnings on the hot path (`ab new`, `ab spawn`). Old-layout directories and orphaned workspaces are surfaced exclusively via `ab dbg orphans`. Users can clean them up with `ab dbg orphans --clean` (all orphans) or `ab dbg orphans --clean <slug>` (a specific slug).
...
7. main.rs CLI changes incl. `DbgCommands::Migrate`, `Orphans`, and `Remove` (depends on 4, 5)
...
11. Manual test: `ab dbg orphans --clean` cleans up all orphaned workspaces (with confirmation prompt)
12. Manual test: `ab dbg orphans --clean <slug>` removes a specific slug directory (with confirmation)
...
22. Manual test: `cd` into a session workspace (linked worktree), run `ab new -s another --git`, verify it creates a workspace for the source repo (not for the worktree itself)
23. Manual test: `cd` into a session workspace (linked worktree), run `ab spawn -s <session> --git`, verify it resolves the source repo and finds the existing workspace
24. Manual test: `ab dbg migrate` correctly handles old-layout workspaces with multi-component relative paths (e.g., `work/project`)
25. Manual test: `ab new my-project` (bare name, no path separator, not a valid path) produces helpful hint about removed repo search
